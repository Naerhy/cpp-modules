...
{
	std::cout << "\033[31m[ClapTrap] Constructor\033[0m" << std::endl;
}

ClapTrap::ClapTrap(ClapTrap const& copy)
	: _name(copy._name), _health(copy._health), _energy(copy._energy), _attack(copy._attack)
{
	std::cout << "\033[31m[ClapTrap] Copy constructor\033[0m" << std::endl;
}

ClapTrap::~ClapTrap(void)
{
	std::cout << "\033[31m[ClapTrap] Destructor\033[0m" << std::endl;
}

ClapTrap& ClapTrap::operator=(ClapTrap const& copy)
{
	std::cout << "\033[31m[ClapTrap] Copy assignement operator\033[0m" << std::endl;
	if (this != &copy)
	{
		_name = copy._name;
...
void ClapTrap::takeDamage(unsigned int amount)
{
	...
	{
		_health = 0;
		std::cout << "ClapTrap <" << _name << "> is in critical condition" << std::endl;
	}
}
